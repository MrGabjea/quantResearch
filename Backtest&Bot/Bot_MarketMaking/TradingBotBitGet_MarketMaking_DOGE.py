...
n = 0.002
size_buffer = 30 #taille du buffer de stock



######################################################################################################################
#%% Fonctions utiles dans la procédure de trading


def update_Orderlist(SO,BO):
    L=pending_order()
    if len(L)==0 :
        return([],[])
    if len(L)==1:
...
while True :
    try:
        ...
        if (CurrentCandleTime>lastCandleTime): # changement de candlestick
            ...
            # mise à jour des ordres d'achat et vente 
            SO,BO = update_Orderlist(Sell_order, Buy_order)
            #annulation des ordres
            print("Starting Update")
            cancel_all_order()
            Sell_order,Buy_order = SO,BO

            try:
                Sell_order.append((candle['openPrice'][0])*(1+n))
                Buy_order.append((candle['openPrice'][0])*(1-n))
                if len(Sell_order)>size_buffer : Sell_order = Sell_order[1:] + [(candle['openPrice'][0])*(1+n)]
                if len(Buy_order)>size_buffer : Buy_order = Buy_order[1:] + [(candle['openPrice'][0])*(1-n)]
                
                if len(pending_order())==0:
                    limit_order(crypto, -size_batch*len(Sell_order), round(np.mean(Sell_order),5))
                    limit_order(crypto, size_batch*len(Buy_order), round(np.mean(Buy_order),5))
            except :
                Sell_order,Buy_order = SO,BO
                print("Problem with placing orders")
            
            time.sleep(1)
            
            invariant = get_Position_Info(crypto)+size_batch*(-len(Sell_order) +len(Buy_order))
            if invariant != 0 :
