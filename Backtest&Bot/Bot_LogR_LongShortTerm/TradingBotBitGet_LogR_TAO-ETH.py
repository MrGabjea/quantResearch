...
size_batch = 2  #taille de la position de base pour les trades 
    ...
    try :
        ...
        if (chrono> 60): # attend 60min
            try :
                ...
                Tao_pos = get_Position_Info(crypto1)
                Eth_pos = get_Position_Info(crypto2)
                print('Current Position : ', Tao_pos/size_batch)
                time.sleep(1)
                
                if (Tao_pos!=(pos*size_batch)):
                    try:
                        market_order(crypto1, round(((pos*size_batch)-Tao_pos),3))
                        time.sleep(1)
                        newEth_pos = -pos*size_batch*prix1/prix2
                        market_order(crypto2, round(newEth_pos-Eth_pos,2)) 
                        print("TAO Position Changed from ", Tao_pos,"to", (pos*size_batch))
