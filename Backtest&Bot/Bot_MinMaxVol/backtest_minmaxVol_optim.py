...
for i in range(12,120,4):
    for k in range(12,120,4):
        ...
        for j in range(len(df)-500,len(df)-300):
...
for i in range(len(df)-500,len(df)-200):
    ...
    if a%24 == 0 :
        ...
        for i in range(12, 120, 4):
            for k in range(12, 120, 4):
                ...
                for j in range(len(dff) - 60, len(dff)):
                    Portefeuille += position * (dff['price'][j] - dff['price'][j - 1])
                    pos_prec = position
                    
                    position = -np.sign(dff['volmax'][j] - dff['volmin'][j])
                    
                    #Portefeuille += -(10 / 10000) * abs(position - pos_prec) * dff['price'][j]
                    if Portefeuille > maxPerf:
                        maxPerf = Portefeuille
                        max_indice = (i, k)
                        #pos_final_max = position
                    if Portefeuille < minPerf:
                        minPerf = Portefeuille
                        min_indice = (i, k)
                        #pos_final_min = position
        traitement_data(df, max_indice[0], max_indice[1])
        traitement_data(df_2, min_indice[0], min_indice[1])
        print(max_indice,min_indice)
        print(maxPerf,minPerf)
...
plt.plot(np.array(df['price'])[400:])
plt.grid()
plt.show()
