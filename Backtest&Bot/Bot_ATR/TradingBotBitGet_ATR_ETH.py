...
def traitement_data(df):
    ...
    df['vol']= (df['closePrice']-df['openPrice'])/1.5
    return None
        ...
        if Intrade:
            if sl>df['low'][i] and sl<df['high'][i]:
                ...
                if tp>df['low'][i] and tp<df['high'][i]:
                    ...
                    Intrade = False
        if not(Intrade):
            if abs(df['vol'][i])>df['ATR'][i] and df['vol'][i]<0:
                k = i
                Intrade = True
                InterDone = False
                
                entry = df['closePrice'][i]+3.5*np.sign(df['vol'][i])*df['ATR'][i]
                inter =df['closePrice'][i] +4.5*np.sign(df['vol'][i])*df['ATR'][i]
                tp = df['closePrice'][i] +6.5*np.sign(df['vol'][i])*df['ATR'][i]
                sl = df['closePrice'][i] -0.5*np.sign(df['vol'][i])*df['ATR'][i]
...
while True :
    try:
        ...
        if (chrono> 30): # attend 30 min
            ...
            if Intrade:
                ...
                if pos ==0:
                    qty = -round(R/(Lprice[0]-Lprice[1]),2)
    ...
    for i in range(max(int(60*(30-chrono)/3),3)): 
        time.sleep(1)    
