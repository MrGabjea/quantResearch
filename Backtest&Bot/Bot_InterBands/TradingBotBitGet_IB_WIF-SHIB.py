...
# Clés API
api_key = 'bg_5672b5100513178f10da99e33a9d821e'
secret_key = 'd70e0e4bd973e9b5dde3800e78e1ad901d09b12af95a3ef08a2b752b3565b0a5'
passphrase = 'KangouQuant0007'
...
def traitement_data(df):
    df['ma_LT']=df['price'].rolling(window = 100).mean()
    df['ma_ST']= df['price'].rolling(window = 70).mean()
    return None
...
def Trading_Step():
    ...
    prix = get_last_Price(crypto1)/get_last_Price(crypto2)
    if (prix_LT>prix_ST):
        if (prix_ST>prix) :
            position = -3
        if prix>prix_LT :
    ...
    try :
        ...
        if (chrono> 60): # attend 60min
            try :
                ...
                Wif_pos = get_Position_Info(crypto1)
                Shib_pos = get_Position_Info(crypto2)
                print('Current Position : ', Wif_pos/size_batch)
                time.sleep(1)
                
                if (Wif_pos!=(pos*size_batch)):
                    try:
                        market_order(crypto1, round((pos*size_batch)-Wif_pos))
                        time.sleep(1)
                        newShib_pos = -pos*size_batch*prix1/prix2
                        market_order(crypto2, round(newShib_pos-Shib_pos)) 
                        print("WIF Position Changed from ", Wif_pos,"to", (pos*size_batch))
